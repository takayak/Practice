		...
		<div class="row">
			<div class="col-sm-12">
				...
				<div class="table-size">
					<table class="table table-bordered table-hover table-striped map2">
						<tr>
							<td>日付</td>
						...
						<tr>
							...
							<%
								for (int i = 0; i < 10; i++) {
										...
										String strdt2 = formatter.format(dt);
										pageContext.setAttribute("strdt2", strdt2);

										Date dt3 = calNow.getTime();
										pageContext.setAttribute("dt3", "dt3");
										String strdt4 = formatter.format(dt3);

										int diff = strdt4.compareTo(strdt2);
										pageContext.setAttribute("diff", diff);
							%>
							<td>
								<%-- 							<fmt:formatDate value="${dt}" pattern="yyyy-MM-dd HH:00:00.0" />
							<p style="color:red"><b><c:out value="${dt2}" /></b></p>--%> <%
 	int a1 = 0;
 %> <%--予約マス判別の条件式など↓ --%> <c:choose>
									<c:when test="${diff>-1}">
						-
						</c:when>
									<c:otherwise>
										<c:forEach var="reserve" items="${reserveList}">
											<c:choose>
												<c:when test="${reserve.date eq strdt2}">
										...
										%>
										<a
											href="addReserve?day=<c:out value="${strdt2}" />&date=<fmt:formatDate value="${dt}" pattern="yyyy/MM/dd HH:00:00.0" />"><img
